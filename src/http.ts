...
export default (chain, port, networkVersion, remote: boolean = false) => {
  ...
  router.get('/peerId', ctx => 
    ctx.body = globalThis.peernet.peerId
  )
  ...
  router.get('/peers', ctx => 
    ctx.body = globalThis.peernet.peers.map(([id, peer]) => id)
  )  
  ...
  router.get('/poolTransactions', async ctx =>
    ctx.body = await globalThis.transactionPoolStore.get()
  )
    router.get('/transactionsInPool', async ctx => 
    ctx.body = await globalThis.transactionPoolStore.length()
  )
    router.get('/transactionPoolSize', async ctx => 
    ctx.body = await globalThis.transactionPoolStore.size()
  )
